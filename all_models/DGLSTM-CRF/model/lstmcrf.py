...
#     '''
#     assert type(sen)==str
#     wordpiece_length=last_hidden_state.size(0)
#     actual_len_wordids=tokenizer.convert_tokens_to_ids(sen.split(' '))
#     wordpiece_len_wordids=tokenizer(sen)['input_ids'][1:-1]
#     assert len(wordpiece_len_wordids)==wordpiece_length>len(actual_len_wordids)
#     new_state=[]
#     i=0
#     j=0
#     print(sen)
#     while i<len(actual_len_wordids):
#         actual_word_id=actual_len_wordids[i]
#         piece_word_id=wordpiece_len_wordids[j]
#         if piece_word_id==actual_word_id:
#             new_state.append(last_hidden_state[j])
#         else:
#             if i==len(actual_len_wordids)-1:
#                 #说明后面的所有wordpiece都是属于最后一个单词的
#                 new_state.append(torch.mean(last_hidden_state[j:],dim=0,keepdims=True))#1,768
#             else:
#                 k=j
#                 while j<wordpiece_length-1 and  wordpiece_len_wordids[j+1]!=actual_len_wordids[i+1]:
#                     j+=1
#                 #说明j+1位置和i+1位置的id是对应的上的
#                 new_state.append(torch.mean(last_hidden_state[k:j+1],dim=0,keepdim=True))
#         i+=1
#         j+=1
#         #此时i和j位置上的id一定是可以对应的上的
#     new_state=torch.vstack(new_state)
#     assert new_state.size()==(len(actual_len_wordids),bert_hidden_size)
#     return new_state
...
class NNCRF(nn.Module):
    ...
    def neural_scoring(self, word_seq_tensor, word_seq_lens, batch_context_emb, char_inputs, char_seq_lens, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, dep_head_tensor, dep_label_tensor, trees=None, config=None):
        """
        ...
        word_emb = self.word_embedding(word_seq_tensor)#(bsz,seq_length,dim1)
        if self.use_char:
            if self.dep_model == DepModelType.dglstm:
        ...
        if self.num_lstm_layer > 1 and (self.dep_model == DepModelType.dglstm):
            for l in range(self.num_lstm_layer-1):
                ...
                packed_words = pack_padded_sequence(feature_out, sorted_seq_len.cpu(), True)
    ...
    def neg_log_obj(self, words, word_seq_lens, batch_context_emb, chars, char_seq_lens, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, tags, batch_dep_label, trees=None, config=None):
        features = self.neural_scoring(words, word_seq_lens, batch_context_emb, chars, char_seq_lens, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, batch_dep_label, trees, config=config)
    ...
    def decode(self, batchInput,config=None):
        ...
        features = self.neural_scoring(wordSeqTensor, wordSeqLengths, batch_context_emb,charSeqTensor,charSeqLengths, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, batch_dep_label, trees, config=config)
        all_scores = self.calculate_all_scores(features)
