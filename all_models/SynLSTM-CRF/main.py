...
def parse_arguments(parser):
    ...
    parser.add_argument('--path_folder', type=str, default='/home/xhsun/Desktop/gitRepositories/ADP2NER/data/BioNLP13CG-IOB')
...
def learn_from_insts(config:Config, epoch: int, train_insts, dev_insts, test_insts):
    ...
    for i in range(1, epoch + 1):
        ...
        # for index in range(len(batched_data)):
            ...
            batch_word, batch_wordlen, batch_context_emb, batch_char, batch_charlen, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, trees, batch_label, batch_dep_label, batch_poslabel = batched_data[index]
            loss = model.neg_log_obj(batch_word, batch_wordlen, batch_context_emb,batch_char, batch_charlen, adj_matrixs, adjs_in, adjs_out, graphs, dep_label_adj, batch_dep_heads, batch_label, batch_dep_label, batch_poslabel, trees, config=config)
...
def test_model(config: Config, test_insts):
    ...
    model_name = "model_files/gcn_{}_hidden_{}_dataset_{}_{}_context_{}.m".format(config.num_gcn_layers, config.hidden_dim, config.dataset, config.affix, config.context_emb.name)
    res_name = "results/gcn_{}_hidden_{}_dataset_{}_{}_context_{}.results".format(config.num_gcn_layers, config.hidden_dim, config.dataset, config.affix, config.context_emb.name)
    model = NNCRF(config)
    model.load_state_dict(torch.load(model_name))
    model.eval()
    test_batches = batching_list_instances(config, test_insts)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SynLSTM CRF implementation")
